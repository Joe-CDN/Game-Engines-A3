...
public class InputPlane : MonoBehaviour, ISaveable
{
    ...
    public Transform barrelSpawnPrefab;
    public Transform spikeSpawnPrefab;

    public Button spawnLabel;
    public Dropdown spawnList;
    public Button undoRedoLabel;
    ...
    {
        if(editMode == false){
            spawnLabel.gameObject.SetActive(false);
            spawnList.gameObject.SetActive(false);
            undoRedoLabel.gameObject.SetActive(false);
            saveBtn.gameObject.SetActive(false);
            loadBtn.gameObject.SetActive(false);
        }
        if(editMode == true){
            spawnLabel.gameObject.SetActive(true);
            spawnList.gameObject.SetActive(true);
            undoRedoLabel.gameObject.SetActive(true);
        ...
        if (Input.GetMouseButtonDown(0) && editMode == true && !EventSystem.current.IsPointerOverGameObject())
        {
            ...
            if (Physics.Raycast(ray, out hitInfo, Mathf.Infinity))
            {
                ...
                if(spawnID == 7)
                {
                    ICommand command = new PlaceCubeCommand(hitInfo.point, barrelSpawnPrefab, "barrelSpawn");
                    CommandInvoker.AddCopmmand(command);
                } 
                if(spawnID == 8)
                {
                    ICommand command = new PlaceCubeCommand(hitInfo.point, spikeSpawnPrefab, "spikeSpawn");
                    CommandInvoker.AddCopmmand(command);
                }                 
            }
        }
